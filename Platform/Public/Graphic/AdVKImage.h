...
namespace ade{
    ...
    public:
        ...
        AdVKImage(AdVKDevice *device,VkExtent3D extent,VkFormat format, uint32_t mipLevels, int useCubeSample);
        ~AdVKImage();

        static bool TransitionLayout(VkCommandBuffer cmdBuffer, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout,uint16_t imageNum = 1,uint16_t levelCount = 1);
