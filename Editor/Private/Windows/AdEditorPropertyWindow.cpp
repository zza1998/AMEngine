...
namespace ade{
    ...
    static bool ResolveComponent(AdEntity *entity, entt::meta_type &meta, entt::meta_any &outInstance){
        auto hasCompFunc = meta.func(HS(FUNC_NAME_HAS_COMPONENT));
        if(!hasCompFunc){
            return false;
        }

        bool hasComp = hasCompFunc.invoke({}, *entity).cast<bool>();
        if(!hasComp){
            return false;
        }
        ...
        outInstance = getCompFunc.invoke({}, *entity);
        return true;
    }
            ...
            if(appContext->scene){
                ...
                if(ImGui::BeginTable("Property Table", 2, tableFlags)) {
                    ...
                    if (AdWidget::BeginGuiField("Name")) {
                        ...
                        strncpy(buffer, pickedEntity->GetName().c_str(), sizeof(buffer));
                        if (ImGui::InputText("##value", buffer, sizeof(buffer))) {
                            pickedEntity->SetName(buffer);
                        }
                        AdWidget::EndGuiField();
                    }
                    ImGui::EndTable();
                ...
                for (const auto &storage: ecsRegistry.storage()){
                    ...
                    if(!ResolveComponent(pickedEntity, meta, instance)){
                    ...
                    if(ImGui::BeginPopupContextItem(componentLabel.c_str())){
                        if(ImGui::Button("Delete Component")){
                            ...
                            if(removeCompFunc){
                                removeCompFunc.invoke({}, *pickedEntity);
                            } else {
                    ...
                    if (ImGui::BeginPopup("AddComponent")){
                        for (const auto &item: notContainComponents){
                            ...
                            if(ImGui::Button(componentLabel.c_str())){
                                ...
                                if(addCompFunc){
                                    addCompFunc.invoke({}, *pickedEntity);
                                } else {
